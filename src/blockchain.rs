...
impl Blockchain {
    ...
    pub fn add_block(&self, block: &Block) {
        ...
        if let Ok(true) = block_tree.contains_key(block.get_hash()) {
            return;
        }
...
pub struct BlockchainIntoIterator<'a> {
    inner: &'a Blockchain,
    current: String,
}
impl<'a> IntoIterator for &'a Blockchain {
    type Item = Block;
    type IntoIter = BlockchainIntoIterator<'a>;

    fn into_iter(self) -> Self::IntoIter {
        let current = self.tip_hash.read().unwrap().clone();
...
impl<'a> Iterator for BlockchainIntoIterator<'a> {
    type Item = Block;
    fn next(&mut self) -> Option<Block> {
        let block_tree = self.inner.db.open_tree(BLOCKS_TREE).unwrap();
        let data = block_tree.get(self.current.clone()).unwrap();
        if data.is_none() {
            return None;
        }
        let block = Block::deserialize(data.unwrap().to_vec().as_slice());
        self.current = block.get_pre_block_hash().clone();
        return Some(block);
    }
}
